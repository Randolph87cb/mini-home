...
`mini-home` 的目标是做一个“双人共享线上小家”的网页原型。当前仓库已经落了一版首屏客厅：可以渲染场景、切换电视 / 单杠 / 信件板互动，并支持桌面小物拖拽和基础布局校验。
...
- 零食和水杯的轻网格吸附拖拽
- 页面内置场景校验卡片与调试模式
- `npm run verify:scene` 自动回归默认场景、电视、信件板、单杠和道具拖拽

## 当前限制

- 当前还是前端原型，尚未接入账号、云端同步和持久化后端。
...
- `npm run verify:scene` 会自动启动预览、检查校验卡片，并回归电视 / 信件板 / 单杠 / 道具拖拽主链路。
